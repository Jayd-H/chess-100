...
func _ready():
	unit_type = "Chancellor"  # This must match your sprite filename
	unit_value = 18  # As per Chess 100 rules
...
func get_movement_pattern():
	...
	# Rook-like movement (horizontal and vertical lines)
	var rook_directions = [
...
# Chancellor attacks the same squares it can move to
func can_attack_square(square_pos):
	...
	# Check if we can attack as a rook (along ranks and files)
	...
	# Check if we can attack as a knight (L-shape)
