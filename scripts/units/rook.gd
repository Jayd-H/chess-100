extends BaseUnit
func _ready():
	unit_type = "Rook"  # This must match your sprite filename
	unit_value = 10  # As per Chess 100 rules
...
# Rook attacks the same squares it can move to
func can_attack_square(square_pos):
	# Must be on board and not the same position as the rook
	if not is_on_board(square_pos) or square_pos == board_position:
		return false
		
	# Check if rook can attack along any horizontal or vertical line
	var dx = square_pos.x - board_position.x
	var dy = square_pos.y - board_position.y
	
	# Check if square is on same row or column
	if dx != 0 and dy != 0:
